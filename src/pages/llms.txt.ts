...
- **Live player-count leaderboard** (${origin}/leaderboards) — the most-played games on Steam
...
- **Game update tracker** (${origin}/updates) — patch notes and season launches for major
...
- **Trending clips** (${origin}/clips) — gaming videos ranked by views per hour since upload
  rather than lifetime views, from public YouTube channel feeds.
- **Free games and deals** (${origin}/deals) — Epic Games Store giveaways and cross-store
  discounts via CheapShark.
- **Cosplay spotlight** (${origin}/cosplay) — recent posts from cosplayers and convention
  photographers on Bluesky, always credited and linked to the original post.
- **Gamer tools** (${origin}/tools) — mouse sensitivity converter, reaction time test, gacha
  pity probability calculator, FPS/frametime reference. All client-side, no data collected.
- **News and analysis** (${origin}/news) — original articles plus a live cluster view showing
...
${lbData.updated ? `\nSnapshot taken ${lbData.updated}. These numbers change continuously; re-fetch ${origin}/data/leaderboard.json for current values.` : ''}

## Machine-readable data

All datasets are served as JSON and may be read directly:

- ${origin}/data/leaderboard.json
- ${origin}/data/updates.json
- ${origin}/data/clips.json
- ${origin}/data/deals.json
- ${origin}/data/cosplay.json
- ${origin}/data/news.json
- ${origin}/data/health.json — per-source fetch status for the last refresh
...
${posts.slice(0, 20).map((p: any) => {
  ...
  return `- [${p.data.title}](${origin}${path}) — ${p.data.description} (${p.data.lang}, ${new Date(p.data.pubDate).toISOString().slice(0, 10)})`;
}).join('\n')}
...
Corrections: ${origin}/contact — factual errors are fixed and the article is stamped with
an updated date.

## Feeds

- ${origin}/rss.xml (English)
- ${origin}/ko/rss.xml (Korean)
